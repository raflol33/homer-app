...
export class LoginComponent implements OnInit {
    ...
    constructor(
        ...
        const savedOAuthToken = sessionStorage.getItem('homer_oauth_token');
        if (savedOAuthToken) {
            sessionStorage.removeItem('homer_oauth_token');
        }
        const search = window.location.search
        const query = decodeURIComponent(search).replace(/(?:returnUrl=\/[a-zA-Z]+\/[a-zA-Z]+)?\?/gm, "");
        const params = new URLSearchParams(query);
        this.oAuthToken = savedOAuthToken || params.get("token");
        if (this.oAuthToken) {
